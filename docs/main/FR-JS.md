...
function replace(itemList, calList, repStr, pattern) {
  ...
  for (var match of matches) {
    ...
    if (newItem != itemMatch) {
      newStr =
        newStr.substring(0, index + offset) +
        "(" +
        newItem +
        ")" +
        newStr.substring(index + offset + itemMatch.length);
...
setTimeout(function () {
  ...
  //将标记值赋给当前类型全选按钮单元格
  var cr = FR.cellStr2ColumnRow(
    _g().getWidgetsByName("all_pass").options.location
  );
  //获取行列号对象
...
## 全选按钮（all_pass\&all_reject）状态改变事件
...
//将标记值赋给当前类型全选按钮单元格
var cr = FR.cellStr2ColumnRow(
  _g().getWidgetsByName("all_reject").options.location
);
//获取行列号对象
_g().setCellValue(0, cr.col, cr.row, isAllChecked);
// 同时互斥的全选标记取消全选
...
1. 在帆软设计器中假设，A1=0.6，则 A2 中用 A1 设置进度条
...
// 弹窗目标报表
var report_address =
  "/webroot/decision/view/report?viewlet=DCH/04hr/data_provided/绩效考核调整事项_填报.cpt&op=write";
...
FR.Msg.alert(
  ...
  function (value) {
    if (value == true) {
      ...
      // 弹窗目标报表
      var report_address =
        "/webroot/decision/view/report?viewlet=DCH/01basic/bi_emp_org/用户账号重置密码.cpt&op=write";
      FR.doHyperlinkByPost({
        //报表路径
        url: report_address,
        //参数
...
setTimeout(function () {
  ...
  $(".x-table td").mouseover(function () {
    ...
    if (tag_style == undefined) {
      ...
      var matched = tag_style
        .replace(/\\s/gi, "")
        .match(/background-color:(.*?);/i);
      var backgroundColor = matched == null ? null : matched[1];
    }
    // 背景色并赋值给全局变量
    window.bg_orgin =
      backgroundColor == null ? window.bg_orgin : backgroundColor;
    // 获取当前鼠标所在单元格的html的id标签值
    var tag_id = $(this).attr("id");
    //截取id值中的行号
    var row = tag_id.substr(0, tag_id.search("-")).replace(/[^0-9]+/gi, "");
    // 设置背景色，标题行不变色
    row <= tb_head_row
      ? null
      : $(this).parent("tr").find("td").css("background-color", bg_select);
  ...
  $(".x-table td").mouseout(function () {
    ...
    row <= tb_head_row
      ...
      : $(this)
          .parent("tr")
          .find("td")
          .css("background-color", window.bg_orgin);
